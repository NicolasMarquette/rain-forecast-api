...
files = {"file": open("./csv_files/test.csv")}
# Réponse attendue
output_expected = [{'2010-10-08/Sydney': 'Yes'}, {'2010-10-09/Sydney': 'No'}]
...
for username, password, expected in zip(api_username, api_password, expected_result):
    ...
    r = requests.post(
        url=f'http://{api_address}:{api_port}/api/v1/predict/clean?models=gbc',
    ...
    output = f'''
============================
  Predict cleaned csv test
============================

request done at "/api/v1/predict/clean?models=gbc"
| username="{username}"
| password="{password}"
...
==>  {test_status}
    ...
    if os.environ.get('LOG') == '1':
        with open('./log/predict_model_clean_csv_test.log', 'a') as file:
            file.write(output)
